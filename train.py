...
def idx_to_label(predictions, correct, idx2label):
    # Convert predictions using nested comprehensions
    label_pred = [
        [idx2label[e] for e in word]
        for sent in predictions
        for word in sent
    ]
    
    # Handle correct labels with walrus operator (Python 3.8+)
    label_correct = [
        [idx2label[e] for e in word]
        for sent in (correct if correct is not None else [])
        for word in sent
    ]
...
def main():
    ...
    split_train = split_text_label(os.path.join(args.data, "eng.train"))
    split_valid = split_text_label(os.path.join(args.data, "eng.testa"))
    split_test = split_text_label(os.path.join(args.data, "eng.testb"))
    ...
    wordSet = {
        ...
        for data in (split_train, split_valid, split_test)
        for labeled_text in data 
        for word, _ in labeled_text
    }

    # Label mappings with explicit '0' prioritization
    sorted_labels = sorted(list(labelSet), key=len) # Ensures '0' first, then length, then alphabetical

    label2Idx = {label: idx for idx, label in enumerate(sorted_labels)}
    idx2Label = sorted_labels  # Direct list access is faster than dictionary lookup

    pickle.dump(idx2Label,open(os.path.join(args.output, "idx2Label.pkl"), 'wb'))
    logger.info("Saved idx2Label pickle file")

    # Word mappings with guaranteed special tokens
    word2Idx = {"PADDING_TOKEN": 0, "UNKNOWN_TOKEN": 1}
    word2Idx.update({word: i+2 for i, word in enumerate(wordSet)})
